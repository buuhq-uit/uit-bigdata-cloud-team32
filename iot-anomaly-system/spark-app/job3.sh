...
NAME="job3-batch-offline"
RUNNER="$ROOT/job3-runner.sh"
LOG_FILE="$LOG_DIR/job3.log"
PID_FILE="$PID_DIR/job3.pid"
...
start() {
  ...
  echo "[start] $NAME (periodic) ..."
  nohup /bin/bash "$RUNNER" >> "$LOG_FILE" 2>&1 &
  echo $! > "$PID_FILE"
  echo "[start] $NAME scheduler started (pid $(cat "$PID_FILE")), log=$LOG_FILE"
}
...
stop() {
  if [[ -f "$PID_FILE" ]]; then
    pid="$(cat "$PID_FILE")"
    if kill -0 "$pid" 2>/dev/null; then
      echo "[stop] $NAME scheduler (pid $pid) ..."
      kill -TERM "$pid" || true
      sleep 2
      pkill -P "$pid" || true
      if kill -0 "$pid" 2>/dev/null; then
        echo "[stop] still running, force kill"
        kill -KILL "$pid" || true
      fi
    else
...
status() {
  if [[ -f "$PID_FILE" ]] && kill -0 "$(cat "$PID_FILE")" 2>/dev/null; then
    echo "[status] $NAME scheduler running (pid $(cat "$PID_FILE"))"
  else
    echo "[status] $NAME scheduler stopped"
  fi
}
